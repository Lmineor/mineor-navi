<template>
  <UContainer class="px-4 py-8 lg:py-12">
    <div class="prose dark:prose-invert a-hover-primary max-w-none">
      ...
      <p>
        {{ $t("officialWebsite") }}：
        <ULink :to="library.url" active-class="text-primary"
          inactive-class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
          {{ library.showUrl || library.url }}
        </ULink>
      </p>
      <h2>{{ $t("baseInfo") }}</h2>
      <UTable :columns="baseInfoColumns" :rows="baseInfos">
        <template #value-data="{ row }">
          <ULink v-if="row.url" :to="row.url" active-class="text-primary"
            inactive-class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
            target="_blank">
            {{ row.value }}
          </ULink>
      ...
      <template v-if="!library.cantPreview">
        ...
        <div class="relative overflow-hidden w-full pb-[56.25%] h-[700px]">
          <iframe :src="library.iframeUrl || library.url" :title="library.name"
            class="absolute top-0 left-0 bottom-0 right-0 w-full h-full" frameBorder="0"
      ...
      <h2>{{ $t("features") }}</h2>
      <UTable :columns="featureColumns" :rows="features">
      ...
      <ul class="p-0">
        <li v-for="(item, index) in randomLibraries" :key="index" class="flex items-center gap-2 mb-10">
          <img :src="`/img/${item.logo}`" :alt="item.name" class="h-12 w-12 drop-shadow-lg mt-0 mb-0 mr-2" />
          <ULink :to="item.url" :title="item.name" active-class="text-primary"
            inactive-class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
            {{ item.finalName }}
          </ULink>
        </li>
...
const finalTitle = `${library.value.name} | Mineor`
